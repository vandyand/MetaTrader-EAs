...
extern bool UseADX = true;//0 = trade inside BB, 1 = useADX for inside/outside, 2 = trade outside BB
extern int ADXPeriod = 25;
...
void OnTick(){
   ...
   if(UseADX==true){//UseADX to determine inside/outside. Problem is, profitable inside and outside BB parameters vary greatly...
      if(((Bid+(0.5*Ask-0.5*Bid)>bb_upper&&ADX_Main>ADXLevel)||(Bid+(0.5*Ask-0.5*Bid)<bb_lower&&ADX_Main<ADXLevel))&&CheckOpenOrders()==false){BuyCond=true;}
      else{BuyCond=false;}
      
      if(((Bid+(0.5*Ask-0.5*Bid)<bb_lower&&ADX_Main>ADXLevel)||(Bid+(0.5*Ask-0.5*Bid)>bb_upper&&ADX_Main<ADXLevel))&&CheckOpenOrders()==false){SellCond=true;}
      else{SellCond=false;}
   }
   /*
   else if(UseADX==0){//Trade inside BB
      if(Bid+(0.5*Ask-0.5*Bid)<bb_lower&&CheckOpenOrders()==false){BuyCond=true;}
      else{BuyCond=false;}
      
      if(Bid+(0.5*Ask-0.5*Bid)>bb_upper&&CheckOpenOrders()==false){SellCond=true;}
      else{SellCond=false;}
   }
   
   else if(UseADX==2){//Trade outside BB
      if(Bid+(0.5*Ask-0.5*Bid)>bb_upper&&CheckOpenOrders()==false){BuyCond=true;}
      else{BuyCond=false;}
      
      if(Bid+(0.5*Ask-0.5*Bid)<bb_lower&&CheckOpenOrders()==false){SellCond=true;}
      else{SellCond=false;}
   }*/
   ...
   if(UseADX==true){
      if(   ((ADX_Main<ADXLevel&&((OrderType()==0&&Bid>bb_main)||(OrderType()==1&&Ask<bb_main)))
            ||(ADX_Main>ADXLevel&&((OrderType()==0&&Ask<bb_main)||(OrderType()==1&&Bid>bb_main))))   
            &&CheckOpenOrders()==true){ExitCond=true;}
      else{ExitCond=false;}
   }
   /*
   if(UsePct==true){
