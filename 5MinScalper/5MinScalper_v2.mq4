...
void OnTick(){
   ...
   for(int i = 1; i <= PastRange_bars; i++){
      if(Low[i] < low_price){low_price = Low[i]; low_bar = i;}
      if(High[i] > high_price){high_price = High[i]; high_bar = i;}  
   }
   ...
   if( OrdersTotal()!=0 && (High[1]<MA(MA_big_period,1)&&Ask>MA(MA_big_period,0)) || (Low[1]>MA(MA_big_period,1)&&Bid<MA(MA_big_period,0)) ){
